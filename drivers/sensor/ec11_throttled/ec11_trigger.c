...
#define DT_DRV_COMPAT custom_ec11_throttled
...
LOG_MODULE_DECLARE(EC11_THROTTLED, CONFIG_SENSOR_LOG_LEVEL);
...
#if defined(CONFIG_EC11_THROTTLED_TRIGGER_OWN_THREAD)
    k_sem_give(&drv_data->gpio_sem);
#elif defined(CONFIG_EC11_THROTTLED_TRIGGER_GLOBAL_THREAD)
...
#ifdef CONFIG_EC11_THROTTLED_TRIGGER_OWN_THREAD
...
#ifdef CONFIG_EC11_THROTTLED_TRIGGER_GLOBAL_THREAD
...
int ec11_init_interrupt(const struct device *dev) {
    ...
    k_thread_create(&drv_data->thread, drv_data->thread_stack,
                    CONFIG_EC11_THROTTLED_THREAD_STACK_SIZE,
                    (k_thread_entry_t)ec11_thread, dev, 0, NULL,
                    K_PRIO_COOP(CONFIG_EC11_THROTTLED_THREAD_PRIORITY), 0, K_NO_WAIT);
#elif defined(CONFIG_EC11_THROTTLED_TRIGGER_GLOBAL_THREAD)
